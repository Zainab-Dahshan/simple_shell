...
return (strdup(node->str + strlen(name) + 1));
}
node = node->next;
}
return (NULL);
...
add_node_end(&info->env_head, strdup(buffer));
add_node_end(&info->env_head, strdup("some_string"));
print_list(info->env_head);
return (0);
}
/**
...
{
add_node_end(head, strdup(environ[x]));
}
}
